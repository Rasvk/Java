...
public class RuntimeIntValue extends RuntimeValue {
	...
	if(intValue == v.getIntValue("== operator", where)) {
   ...
   public RuntimeValue evalAdd(RuntimeValue v, AspSyntax where) {
	...
	return new RuntimeBoolValue(!(v instanceof RuntimeIntValue));
    }
	...
	if (v instanceof RuntimeIntValue) {
	    return new RuntimeIntValue(Math.floorMod(intValue, 
						     v.getIntValue("% operand",where)));
	} else if (v instanceof RuntimeFloatValue) {
		return new RuntimeFloatValue((intValue -
					      v.getFloatValue("% operand",where) * Math.floor(intValue/v.getFloatValue("% operand", where))));
	}
	runtimeError("Type error for %.", where);
	return null;  // Required by the compiler.
    }
